...
def read_dataset():
    ...
    # Encode character labels into integer i.e 1 or 0 (One hot encode)
    encoder.fit(y)
...
def multilayer_perceptron(x, weights, biases):
    ...
    # First Layer performs matrix multiplication of inputs with weights
...
def main():
    ...
    print("Marvellous Infosystems : train_x.reshape", train_x.shape)
    ...
    # Define the parameters which are required for tensors ie hyperparameters
    ...
    # As we have two clases R & M
    n_class = 2

    # Path which contains model files
    model_path = "TensorFlowProject"

    # Define the number of Hidden layers and the number of perceptron for each layer
    # Number of Hidden Layers are 4
    # Neurons in each layer are 60
    ...
    # Create Variables which contains random values
    ...
    training_step = tf.compat.v1.train.GradientDescentOptimizer(
        learning_rate).minimize(cost_function)

    # Launch the graph
    sess = tf.compat.v1.Session()
    File_Writer = tf.compat.v1.summary.FileWriter("Demo", sess.graph)
    sess.run(init)
    ...
    # Display graph for Loss calculation
    plt.plot(range(len(cost_history)), cost_history)
    plt.title("Marvellous Infosystems : Loss calculation")
